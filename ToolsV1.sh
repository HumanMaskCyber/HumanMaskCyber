
#!/bin/bash
#version 1.0
#Mao Ngapain?
#Mao Recode?
#Utamakan Izin Dulu^_^
clear
#variabels
b='\033[34;1m'
g='\033[32;1m'
p='\033[35;1m'
c='\033[36;1m'
r='\033[31;1m'
w='\033[37;1m'
y='\033[33;1m'
sleep 1

echo
echo
echo
echo
cowsay -f eyes " WELCOME TO MY WORLD " | lolcat
sleep 5
clear
cowsay -f eyes " HumanMask " | lolcat

figlet HumanMask | lolcat

echo "[+]""▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼""[+]"
echo " ▶ ""      ◆◆◆◆◆◆      ♛ Toxic City Attacker ♛      ◆◆◆◆◆◆     "" ◀ "
echo " ▶ ""▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼"" ◀ "
echo " ▶ "" Author      : "" HumanMask                               "" ◀ "
echo " ▶ "" Instagram   : "" _humanmask_                             "" ◀ "
echo " ▶ "" Motivation  : "" No System Is Safe                       "" ◀ "
echo " ▶ "" Thanks To   : "" All Member Toxic City Attacker          "" ◀ "
echo "[+]""▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲""[+]"

###################################################
# CTRL + C
###################################################
trap ctrl_c INT
ctrl_c() {
clear
echo -b $g"[#]> (Ctrl + C ) Detected, Trying To Exit ...."
echo -b $g"[#]> GUNAKAN TOOLS INI DENGAN BIJAK !!! "
sleep 1
echo ""
echo -b $g"[#]> HumanMask"

echo -b $g"[#]> See You Again :)..."
sleep 1
exit
}

lagi=1
while [ $lagi -lt 6 ];
do
echo""
echo""
echo""
echo $c"1. Locator "
echo $c"■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■"
echo $r"2. BruteForce Facebook"
echo $r"■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■"
echo $g"3. Cloning Yahoo "
echo $g"■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■"
echo $y"4. Bot Komen Facebook "
echo $y"■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■"
echo $b"5. Auto Report Facebook "
echo $b"■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■"
echo $w"6. Profile Guard Facebook "
echo $w"■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■"
echo $r"7. Open Source Information Facebook (OSIF) "
echo $r"■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■"
echo $c"8. Phising All Social Media (Weeman)"
echo $c"■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■"
echo $r"00. Keluar "
echo $g
read -p " Pilih No : " pil;
case $pil in
1) clear
toilet -f standard " HumanMask " -F gay
git clone https://github.com/thelinuxchoice/locator.git
cd locator
bash locator.sh

;;

2) clear
toilet -f standard " HumanMask " -F gay
git clone https://github.com/IqbalzNoobs/fb-brute
cd fb-brute
python2 brute.py

;;

3) clear
toilet -f standard " HumanMask " -F gay
git clone https://github.com/FR13ND8/EmailVuln
python2 vuln.py

;;

4) clear
toilet -f standard " HumanMask " -F gay
pip2 install mechanize
git clone https://github.com/Senitopeng/Botkomena.git
cd Botkomena
python2 botkomena.py

;;

5) clear
toilet -f standard " HumanMask " -F gay
git clone https://github.com/IlayTamvan/Report
cd Report
unzip Report.zip
python2 Report.py

;;

6) clear
toilet -f standard " HumanMask " -F gay
git clone https://github.com/FR13ND8/ProfileGuardFb
cd ProfileGuardFb
php guard.php

;;

7) clear
toilet -f standard " HumanMask " -F gay
git clone https://github.com/CiKu370/OSIF
cd OSIF
python2 osif.py


;;




8) clear
toilet -f standard " HumanMask " -F gay
git clone https://github.com/evait-security/weeman.git
cd weeman
python2 weeman.py

;;

00) toilet -f standard " HumanMask " -F gay
echo $g"Keluar....."
exit
;;

esac
done
done


■■■■■■■■■■■■■■■■"
echo $r"00. Keluar "
echo $g
read -p " root@HumanMask:~# " pil;

case $pil in
1) clear
toilet -f standard " HumanMask " -F gay
git clone https://github.com/thelinuxchoice/locator.git
cd locator
bash locator.sh

;;

2) clear
toilet -f standard " HumanMask " -F gay
git clone https://github.com/IqbalzNoobs/fb-brute
cd fb-brute
python2 brute.py

;;
read -p " root@HumanMask:~# " pil;

case $pil in
1) clear
toilet -f standard " HumanMask " -F gay
git clone https://github.com/thelinuxchoice/locator.git
cd locator
bash locator.sh

;;

2) clear
toilet -f standard " HumanMask " -F gay
git clone https://github.com/IqbalzNoobs/fb-brute
cd fb-brute
python2 brute.py

;;

3) clear
toilet -f standard " HumanMask " -F gay
git clone https://github.com/FR13ND8/EmailVuln
cd EmailVuln
python2 vuln.py

;;

4) clear
toilet -f standard " HumanMask " -F gay
pip2 install mechanize
git clone https://github.com/Senitopeng/Botkomena.git
cd Botkomena
python2 botkomena.py

;;

5) clear
toilet -f standard " HumanMask " -F gay
git clone https://github.com/IlayTamvan/Report
cd Report
unzip Report.zip
python2 Report.py

;;

6) clear
toilet -f standard " HumanMask " -F gay
git clone https://github.com/FR13ND8/ProfileGuardFb
cd ProfileGuardFb
php guard.php


;;

7) clear
toilet -f standard " HumanMask " -F gay
git clone https://github.com/CiKu370/OSIF
cd OSIF
python2 osif.py

;;

8) clear
toilet -f standard " HumanMask " -F gay
git clone https://github.com/evait-security/weeman.git
cd weeman
python2 weeman.py

;;

00) toilet -f standard " HumanMask " -F gay
echo $g"Keluar....."
exit
;;

esac
done
done


■■■■■■■■■■■■■■■■"
echo $r"00. Keluar "
echo $g
read -p " root@HumanMask:~# " pil;

case $pil in
1) clear
toilet -f standard " HumanMask " -F gay
git clone https://github.com/thelinuxchoice/locator.git
cd locator
bash locator.sh

;;

2) clear
toilet -f standard " HumanMask " -F gay
git clone https://github.com/IqbalzNoobs/fb-brute
cd fb-brute
python2 brute.py

;;

3) clear
toilet -f standard " HumanMask " -F gay
git clone https://github.com/FR13ND8/EmailVuln
cd EmailVuln
python2 vuln.py

;;

4) clear
toilet -f standard " HumanMask " -F gay
pip2 install mechanize
git clone https://github.com/Senitopeng/Botkomena.git
cd Botkomena
python2 botkomena.py

;;


5) clear
toilet -f standard " HumanMask " -F gay
git clone https://github.com/IlayTamvan/Report
cd Report
unzip Report.zip
python2 Report.py

;;

6) clear
toilet -f standard " HumanMask " -F gay
git clone https://github.com/FR13ND8/ProfileGuardFb
cd ProfileGuardFb
php guard.php



;;

7) clear
toilet -f standard " HumanMask " -F gay
git clone https://github.com/CiKu370/OSIF
cd OSIF
python2 osif.py

;;

8) clear
toilet -f standard " HumanMask " -F gay
git clone https://github.com/evait-security/weeman.git
cd weeman
python2 weeman.py

;;

00) toilet -f standard " HumanMask " -F gay
echo $g"Keluar....."
exit
;;

esac
done
done


■■■■■■■■■■■■■■■■"
echo $r"00. Keluar "
echo $g
read -p " root@HumanMask:~# " pil;

case $pil in
1) clear
toilet


